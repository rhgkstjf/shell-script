...
#echo "----------------로그 데이터 전송 원격 시작------------------"
#ssh [user@ip] 'sh [UserHomePath]/AutoLog/LogInput.sh'
#sleep 5m
#echo "----------------로그 데이터 전송 완료--------------------"
...
echo "---------------웹 서버에 데이터 전송 중--------------"
ssh -p [Port] [WebServer@ip] "echo [Pwd] | sudo -S mkdir [WebServerPath]/FLASKAPPS/Data/$P"
sleep 5s
ssh -p [Port] [WebServer@ip] "sh [WebServerUserHomePath]/DataLoad.sh"
